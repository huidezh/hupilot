...
'<div class="ai-chat-settings-row ai-chat-settings-row-nested" id="ai-chat-shell-install-row" style="display:none">' +
  '<details style="font-size:12px">' +
    ...
    '<p style="margin:6px 0;color:#888">出现英文询问是否要安装时，按Y键确认，安装完成后重启浏览器，然后可在对话界面点击计算机按钮启用功能，本功能启用时为保证使用体验会同时启用联网功能。</p>' +
  '</details>' +
'</div>' +
'<div class="ai-chat-settings-row ai-chat-settings-row-nested" id="ai-chat-shell-test-row" style="display:none">' +
  ...
  '<div id="ai-chat-shell-skills-dir" style="font-size:12px;margin-top:4px;display:none"></div>' +
'</div>' +
    ...
    document.getElementById('ai-chat-shell-toggle').addEventListener('click', function() {
      ...
      if (session.shellHostEnabled && !session.webSearchEnabled) {
        session.webSearchEnabled = true;
        var searchBtn = document.getElementById('ai-chat-search-toggle');
        if (searchBtn) {
          searchBtn.classList.toggle('ai-chat-search-off', false);
          searchBtn.classList.toggle('ai-chat-search-on', true);
        }
      }
      saveSessions();
        ...
        var updateShellStatus = function() {
          ...
          callShellHost('tools/call', { name: 'shell_status', arguments: {} }).then(function() {
            ...
            checkShellHostVersion().then(function(v) {
              ...
              var skillsDirEl = document.getElementById('ai-chat-shell-skills-dir');
              if (skillsDirEl && window.__skillsDir) {
                skillsDirEl.style.display = '';
                skillsDirEl.innerHTML = '技能文件夹：<code style="user-select:all;cursor:pointer;color:#555" title="点击复制路径">' + window.__skillsDir + '</code>';
                var codeEl = skillsDirEl.querySelector('code');
                if (codeEl) {
                  codeEl.addEventListener('click', function() {
                    navigator.clipboard.writeText(window.__skillsDir).then(function() {
                      showToast('已复制技能文件夹路径');
                    });
                  });
                }
              }
            ...
            if (infoEl) infoEl.innerHTML = '';
            var skillsDirEl = document.getElementById('ai-chat-shell-skills-dir');
            if (skillsDirEl) skillsDirEl.style.display = 'none';
          });
        };
      ...
      if (pageContent.length > maxChars) pageContent = pageContent.substring(0, maxChars);
        ...
        if (systemParts.length > 0) {
          ...
          if (session.shellHostEnabled) {
            systemText += '\n\n### Shell 工具\nShell 已连接。可通过工具调用（function calling）执行本地命令和 Python 代码。\n\n可用工具：\n- shell_exec: 执行本地 Shell/PowerShell 命令\n- python_exec: 执行自定义 Python 代码（已有 .py 脚本用 shell_exec）\n- shell_status: 查看系统环境\n- local_folder_pick: 选择本地文件夹\n\n路径中的反斜杠请用正斜杠替代，例如 C:/Users/\n\n### 代码执行规则\n- 回复文本中严禁出现任何可执行代码（Python、Shell、命令等），所有代码必须通过 tool call 执行\n- Python 的执行方式：自己编写的小段代码用 python_exec 工具；技能中已有的 .py 脚本文件用 shell_exec（python.exe 脚本路径 参数）执行。两种方式禁止混用\n- 技能文档中的脚本路径和代码示例仅供参考调用，禁止复制到回复文本中\n- 执行 python_exec 后如果出错，在上一轮的代码基础上增量修正，不要重新写完整代码\n- 执行删除、清空等危险操作前，必须先向用户详细说明涉及的文件和风险，获得用户明确同意后再执行';
            if (window.__skillList && window.__skillList.length > 0) {
              systemText += '\n\n### 技能（优先于直接工具调用）\n当用户问题与以下某个技能匹配时，严格按此流程：\n\n1. **激活** — 在回复中输出 <skill name="技能名"/>，等待技能文档注入\n2. **执行** — 技能文档中的 .py 脚本用 shell_exec 运行，自定义代码用 python_exec\n\n禁止跳过技能直接执行。可用技能：\n';
              for (var si = 0; si < window.__skillList.length; si++) {
                var sk = window.__skillList[si];
                systemText += '\n- **' + sk.name + '**: ' + (sk.description || '');
              }
              if (window.__skillsDir) {
                systemText += '\n\n技能脚本路径: ' + window.__skillsDir + '/技能名/scripts/脚本名';
              }
            }
          }
      ...
      if (s.deskPetAlways === true && floatingBtn) {
        ...
        startFloatingTimer();
              ...
              if (shellTcs.length > 0) {
                ...
                if (result && result.content && result.content.trim()) {
                  ...
                  if (msgEls && msgEls.contentEl) {
                    showDoneLoading();
                    requestAnimationFrame(function() { updateLastAssistantMessage('已执行Shell命令，请等待输出结果。'); scrollToBottom(); });
                  } else {
                ...
                var shellPromises = shellTcs.map(function(tc) {
                  ...
                  return callShellHost('tools/call', { name: tc.function.name, arguments: args }).then(function(res) {
                    ...
                    var ruleMsg = '【强制规则】回复文本中严禁出现任何可执行代码。Python 代码的执行方式：自定义代码用 python_exec，已有 .py 脚本用 shell_exec。两种方式禁止混用。\n\n';
                    return { role: 'tool', tool_call_id: tc.id, content: ruleMsg + text };
                  }).catch(function(err) {
                    var ruleMsg = '【强制规则】回复文本中严禁出现任何可执行代码。Python 代码的执行方式：自定义代码用 python_exec，已有 .py 脚本用 shell_exec。两种方式禁止混用。\n\n';
                    return { role: 'tool', tool_call_id: tc.id, content: ruleMsg + 'Error: ' + err.message };
                  });
                });
            ...
            if (skillMatch) {
              ...
              if (msgEls && msgEls.contentEl) { updateLastAssistantMessage(cleanSkillContent); showDoneLoading(); removeTyping(); }
              else { appendMessageDOM('assistant', cleanSkillContent); removeTyping(); showDoneLoading(); }
              return callShellHost('tools/call', { name: 'skill_get', arguments: { name: skillName } }).then(function(res) {
                ...
                if (mdContent) {
                  ...
                  if (window.__skillsDir) {
                    ...
                    skillPathNote = '\n\n### 路径说明\n脚本目录: ' + skillDir + '/scripts\n已有的 .py 脚本用 shell_exec 运行（python.exe 脚本路径 参数），自定义代码用 python_exec';
                  }
                  var injectContent = '\n\n【强制规则】回复文本中严禁出现任何可执行代码。Python 代码：自定义代码用 python_exec，已有 .py 脚本用 shell_exec。\n\n### ' + skillName + ' 技能文档\n以下是为当前任务加载的技能文档。请严格遵循其中的指令来完成任务。\n\n' + mdContent + skillPathNote;
                  var sysFound = false;
                  for (var mi = 0; mi < messages.length; mi++) {
                    if (messages[mi].role === 'system') {
                      messages[mi].content += injectContent;
                      sysFound = true;
                      break;
                    }
                  }
                  if (!sysFound) messages.unshift({ role: 'system', content: injectContent });
                  return doSendLoop(messages, round + 1, roundTools);
                }
              });
            }
