// === Outlook 邮件优化器 ===
// 匹配新版 Outlook 网页版（cloud.microsoft / office.com）
...
registerOptimizer(
  /^https:\/\/outlook\.(office|cloud\.microsoft)(\.com)?\/mail\//,
  {
    name: 'Outlook 邮箱',
    ...
    extractContent: function() {
      return readAISettings().then(function(settings) {
        ...
        var subject = qt('[id$="_SUBJECT"] .JdFsz');
        var from = qt('[id$="_FROM"]');
        var date = qt('[data-testid="SentReceivedSavedTime"]');
        var to = qt('[data-testid="RecipientWell"]');
        var body = '';

        var bodyEl = q('[data-test-id="mailMessageBodyContainer"] [role="document"]');
        if (bodyEl) body = bodyEl.innerText || bodyEl.textContent || '';

        if (!subject) {
          var headingEl = q('[role="main"] [role="heading"]');
          if (headingEl) subject = headingEl.textContent.trim();
        }
        if (!body) {
          var mainEl = q('[role="main"]');
          if (mainEl) {
            var allText = mainEl.innerText || mainEl.textContent || '';
    ...
    getQuickActions: function() {
      return [
        ...
        },
        {
          id: 'reply',
          label: '回复',
          prompt: '{content}\n\n{recipient}\n\n用不超过 3 个简洁的要点概括关键信息，然后使用此邮件语言撰写一份快速回复草稿，回复内容要符合用户语气和意图，必要时做出合理的假设，用户为【{userInfo}】：'
        },
        {
          id: 'keypoints',
          label: '要点',
