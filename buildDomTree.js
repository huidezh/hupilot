window.buildDomTree = (
  ...
  function buildDomTree(node, parentIframe = null, isParentHighlighted = false, depth = 0) {
    ...
    if (node.nodeType === Node.ELEMENT_NODE && node.id && (node.id.startsWith('ai-chat-') || node.id.startsWith('he-'))) {
      return null;
    }
    if (node.nodeType === Node.ELEMENT_NODE && node.classList && node.classList.contains('he-overlay')) {
      return null;
    }
